...
import { requireVenueMember, isAllowedImageType, imageExtForMime } from "@/lib/security/guards";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!isAllowedImageType(file.type)) {
      return NextResponse.json({ error: "Unsupported image type — use JPEG, PNG, WebP, GIF or AVIF." }, { status: 415 });
    }
