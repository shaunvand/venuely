...
export default async function CouplePortalPage({ params }: { params: Promise<{ wedding: string }> }) {
  ...
  const [wedRes, venRes, catRes, rentRes, roomRes, vendRes, galRes] = await Promise.all([
    ...
    db.from("catalogue_items").select("id, category, name, description, image_url, cost_treatment, sort_order").eq("venue_id", vId).eq("active", true).order("sort_order"),
  ...
  const catalogue = (catRes.data ?? []).map((c) => ({ id: c.id, category: c.category, name: c.name, description: c.description ?? "", img: (c.image_url as string) ?? null, included: (c.cost_treatment as string) === "included" }));
