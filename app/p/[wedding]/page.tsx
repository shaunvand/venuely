...
export default async function CouplePortalPage({ params }: { params: Promise<{ wedding: string }> }) {
  ...
  const [wedRes, venRes, catRes, rentRes, roomRes, vendRes, galRes, tableRes, areaRes, areaPriceRes, areaImgRes, areaGroupRes, seasonRes, guestsRes] = await Promise.all([
    ...
    db.from("rental_items").select("id, category, name, description, price, image_url, cost_treatment, commission_value, commission_type, stock_total, sort_order").eq("venue_id", vId).eq("active", true).order("sort_order"),
  ...
  const rentals = (rentRes.data ?? []).map((r) => ({ id: r.id, category: r.category, name: r.name, description: r.description ?? "", img: (r.image_url as string) ?? null, price: applyMarkup(num(r.price), r.commission_value, r.commission_type), included: (r.cost_treatment as string) === "included", stock: r.stock_total == null ? null : Number(r.stock_total) }));
