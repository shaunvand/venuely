...
export async function addRoom(venueId: string, formData: FormData) {
  ...
  await supabase.from("accommodation_rooms").insert({
    venue_id: venueId,
...
export async function toggleRoomActive(roomId: string, active: boolean) {
  const supabase = await createClient();
  await supabase.from("accommodation_rooms").update({ active }).eq("id", roomId);
  revalidatePath("/venue/accommodation");
}

export async function deleteRoom(roomId: string) {
  const supabase = await createClient();
  await supabase.from("accommodation_rooms").delete().eq("id", roomId);
  revalidatePath("/venue/accommodation");
}
