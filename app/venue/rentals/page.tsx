...
export default async function VenueRentals() {
  ...
  const { data: items } = await supabase
    .from("rental_items")
    .select("id, category, name, description, price, stock_total, active")
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-semibold">Rentals · {venue.name}</h1>

      <form action={addRental.bind(null, venue.id)} className="flex flex-wrap gap-2 border rounded p-4">
        ...
        <input name="price" type="number" step="0.01" required placeholder="Price" className="border rounded px-3 py-2 w-28" />
        <input name="stock_total" type="number" min="1" defaultValue="1" className="border rounded px-3 py-2 w-20" />
      ...
      <table className="w-full text-sm">
        <thead className="text-left text-gray-500">
          <tr><th>Category</th><th>Name</th><th>Price</th><th>Stock</th><th>Active</th><th></th></tr>
        </thead>
        <tbody>
          {items?.map((i) => (
