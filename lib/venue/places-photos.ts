...
import { VENUE_LOCATIONS } from "@/app/api/venue/gallery/route";

// Core logic for pulling a venue's own photos from Google Places into the
// "Your Venue" gallery, factored out of the API route so it can be invoked
// directly server-side (no HTTP round-trip / auth needed) — e.g. an automatic
// pre-import. The route still wraps this for the client trigger.
//
// IMPORTANT — keys: the public NEXT_PUBLIC_GOOGLE_MAPS_API_KEY is HTTP-referrer
// restricted, which Google forbids for server-side web-service calls. This needs
// a SEPARATE server key in GOOGLE_PLACES_API_KEY (no referrer restriction).
...
export type ImportPhotosResult = {
  ...
  message: string;
  warnings?: string[];
  // Set when the import couldn't run (config/lookup failure). Callers that want
  // to surface an HTTP status (the route) map this to a code.
  error?: string;
  errorStatus?: number;
};
...
// Import the venue's Google Places photos into the gallery. Idempotent: skips
// slots already imported ("Google photo N" labels). Does NOT auth the caller —
// callers (the route) gate access before invoking.
export async function importPlacesPhotos(venueId: string): Promise<ImportPhotosResult> {
  ...
  const { data: venue } = await sb
    ...
    .eq("id", venueId)
    .single();
  if (!venue) return { ok: false, added: 0, message: "Venue not found", error: "Venue not found", errorStatus: 404 };
  ...
  if (!placeId) {
    ...
    if (!q) return { ok: false, added: 0, message: "No google_place_id and no name/address to search by.", error: "No google_place_id and no name/address to search by.", errorStatus: 422 };
    ...
    if (findData.status !== "OK" || !findData.candidates?.length) {
      const e = `Google could not find this venue (${findData.status || findRes.status}). ${findData.error_message ?? ""}`.trim();
      return { ok: false, added: 0, message: e, error: e, errorStatus: 502 };
    }
  ...
  if (detData.status !== "OK") {
    const e = `Google Place Details ${detData.status || detRes.status}. ${detData.error_message ?? ""}`.trim();
    return { ok: false, added: 0, message: e, error: e, errorStatus: 502 };
  }
  const photos = (detData.result?.photos ?? []) as Array<{ photo_reference: string; html_attributions?: string[] }>;
  if (photos.length === 0) {
    return { ok: true, added: 0, message: "Google has no photos for this venue." };
  }
